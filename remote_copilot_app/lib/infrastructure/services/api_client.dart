...
class ApiClient {
  ApiClient({required http.Client client, required SseClient sseClient, required String baseUrl})
    : _client = client,
  ...
  Future<Map<String, dynamic>> postJson(String path, Map<String, dynamic> body) async {
    final response = await _client.post(_buildUri(path), headers: const {'Content-Type': 'application/json'}, body: jsonEncode(body));
    return _decodeObject(response);
  }

  Future<Map<String, dynamic>> putJson(String path, Map<String, dynamic> body) async {
    final response = await _client.put(_buildUri(path), headers: const {'Content-Type': 'application/json'}, body: jsonEncode(body));
    return _decodeObject(response);
  }
  ...
  Stream<ConversationStreamEvent> openConversationStream(String conversationId) {
    return _sseClient.connect(_buildUri('/api/conversations/$conversationId/events'));
  }
  ...
  Map<String, dynamic> _decodeObject(http.Response response) {
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw ApiException('The server returned ${response.statusCode}: ${response.body}');
    }
